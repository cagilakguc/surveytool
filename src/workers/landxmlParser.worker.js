...
function tagBlocks(source, localName) {
...
function firstTagBlock(source, localName) {
  return tagBlocks(source, localName)[0] ?? null
}

function firstTagAttributes(source, localName) {
  const name = escapeRegExp(localName)
  const expression = new RegExp(
    `<(?:[\\w.-]+:)?${name}\\b([^>]*)\\/?>`,
    "i",
...
function pointFromTag(source, localName) {
...
function sampleCurve(body, attributes) {
...
function parseAlignmentPoints(alignmentBody) {
...
function parseSurface(block, index) {
...
function parseLandXmlText(xmlText, fileName) {
  ...
  const alignments = tagBlocks(xmlText, "Alignment").map((block, index) => ({
